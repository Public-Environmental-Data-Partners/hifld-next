...
class GeoServerClient:
    ...
    async def delete_store(self, workspace: str, store_name: str, recurse: bool = True) -> bool:
    ...
    async def create_geopackage_store(
        ...
        if geopackage_url.startswith("http://") or geopackage_url.startswith("https://"):
            # For HTTP URLs, check if we're in a Docker environment
            ...
            if "localhost" in geopackage_url or "127.0.0.1" in geopackage_url:
                ...
                logger.warning(
                    f"GeoPackage store with HTTP URL will likely fail. "
                    f"GeoServer's GeoPackage plugin requires file:// URLs. "
                    f"For Cloud Run, use GeoParquet (supports HTTP URLs) instead."
                )
    ...
    async def create_geoparquet_store(
        self, workspace: str, store_name: str, geoparquet_url: str, s3_endpoint: Optional[str] = None
    ) -> bool:
        """
        ...
        **Using S3 URLs with SeaweedFS:**
        ...
        if await self.store_exists(workspace, store_name):
            logger.info(f"Store '{store_name}' already exists in GeoServer, deleting to update URL...")
        ...
        if s3_endpoint and geoparquet_url.startswith("s3://"):
            ...
            connection_params.extend([
                ...
                {"@key": "awsRegion", "$": "us-east-1"},  # Dummy region for S3-compatible storage
