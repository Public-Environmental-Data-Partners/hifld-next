...
descriptor and PMTiles URL. Only the query variant contains a query ID and
server MVT configuration. The query workspace owns the corresponding token in
private React state. The token is omitted from WebMCP results, URLs, analytics,
and logs.

Multiple query layers may have different tokens even though the server returns
different query IDs. The query service returns a random, non-secret query ID
bound to the signed token and uses it in the resource path. The webapp maintains
an in-memory query-ID-to-token map. MapLibre's `transformRequest` uses the query
ID to attach the correct token header to each tile request. This gives MapLibre
distinct cache keys without placing a query token in the URL. The server rejects
a path ID that does not match the signed token, and the browser sends no tile
request for an unknown local ID.
...
### Query layers in the visible workspace

A mapped query is an ordinary visible workspace layer, not a MapLibre-only
source. The same ordered `loadedLayers` state drives MapLibre, the existing
Loaded layers list, and the Style layers section for both catalog and query
variants.

`run_dataset_query` accepts an optional plain-text `layer_label` of at most 80
characters. When omitted, the UI assigns a sequential label such as
`Query result 1`; it does not derive a label from raw SQL. A query layer row in
the Loaded list shows:

- The label and a `Query result` badge.
- A bounded source-alias and geometry-column summary.
- Its `loading`, `ready`, or `error` map status.
- The same visibility and remove controls as a catalog layer.
- A `View results` action in place of `Open file`; it focuses that query's
  result table.

Query vector sublayers appear in the existing Style layers section and use the
same field-aware styling controls. Only scalar properties retained in the MVT
output are offered as style fields. Visibility, ordering, and style commands
work across catalog and query layers through the same IDs and command path.

The query result panel provides the reverse link: an `On map` state and action
to focus its layer, or a human `Show on map` action if the layer was removed.
Removing a query layer clears only its MapLibre source, layer presentation, and
style state. It retains the query result, current table page, and transient
token so the user can keep paging or show it on the map again. Agents should use
visibility for temporary hiding; rerunning `run_dataset_query` remains their
path to recreate a removed query visualization without adding another WebMCP
tool.

If the table query succeeds but map setup fails, the result remains usable and
the tool returns success with a bounded warning. The layer row exposes the map
error without leaking server details, and no invisible or partially registered
layer is reported as ready.
...
- `POST /api/queries/{query_id}/pages` accepts `offset` and `page_size` and
  receives the signed token in `X-HIFLD-Query-Token`.
- `GET /api/queries/{query_id}/tiles/{z}/{x}/{y}.mvt` receives the signed token
  in `X-HIFLD-Query-Token`.

These endpoints return the existing structured query/page contracts and stable
error codes. The create response adds the non-secret `query_id`, which is bound
to the signed token and verified on page and tile requests. The ID is an opaque
resource identifier, not server-side session state; no query or result is
stored. These routes do not add alternate source resolution, SQL policy,
worker, serialization, token, or tile implementations. Existing MCP and tile
routes remain available for their current callers.
...
Query responses contain an absolute public tile URL under
`/api/queries/{query_id}/tiles/{z}/{x}/{y}.mvt`, generated from
...
The WebMCP result exposes the server-issued opaque `query_id` for selecting
among current in-tab results. The ID is safe to place in a path but is useless
without its corresponding signed token. Neither survives reload in the
browser.
...
| Tool | Inputs | Behavior | Registration |
|---|---|---|---|
| `run_dataset_query` | source refs, SQL, result limit, optional geometry settings, `show_on_map`, and `layer_label` | Execute server-side, render the first page, optionally add its spatial MVT layer, focus the result panel, and return a compact preview | map workspace mounted |
...
After validating the result, the UI creates the result panel first and then
appends a `QueryMvtLayer` to the same Loaded and Style layer state used by
catalog layers. The tool resolves after the layer row is visible and its map
status is stable; it does not wait for every tile to load.
...
- Query execution displays the result panel and any requested query layer in
  the visible layer controls before returning.
...
- Mapped query results appear in the Loaded and Style layer controls with a
  query badge, status, and bidirectional link to their result table.
- Query visibility, styling, ordering, and removal use the shared layer command
  path; removal preserves table paging and the human `Show on map` action.
- A successful table query with failed map setup returns a bounded warning and
  never creates a ghost ready layer.
...
- Reject a page or tile path whose `query_id` does not match the signed token.
...
5. Execute a two-source spatial join, automatically add its MVT layer, verify
   that the visible layer row can be styled, reordered, hidden, and linked back
   to its table, and retain the existing catalog layers.
...
- Spatial query results render through bbox-constrained MVT tiles alongside
  catalog PMTiles layers and appear in the same visible layer and style UI.
- Query table and map-layer state remain linked in both directions, and
  removing the visualization does not discard the pageable result.
