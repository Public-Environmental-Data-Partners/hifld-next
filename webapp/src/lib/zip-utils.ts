...
export async function createZipFromUrls(
  ...
  if (failedFiles.length > 0) {
    const errorMessages = failedFiles.map((f) => `${f.name}: ${f.error?.message || "Unknown error"}`).join("\n");

    const error = new Error(`Failed to download ${failedFiles.length} of ${totalFiles} files:\n${errorMessages}`);
 ...
 */
export function extractShapefileUrls(
  sources: Array<{
    id?: number;
    url?: string;
  ...
  for (const source of sources) {
    ...
    const hasShapefileExtension = shapefileExtensions.some((ext) => filename.toLowerCase().endsWith(ext));
