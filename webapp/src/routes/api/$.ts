...
function apiNotFound(request: Request) {
  ...
  let detail =
    "No handler for this path. Catalog routes return STAC Catalog and Collection documents, but this is not a STAC API or OGC API-Features item service: there are no /items, /features, /download, or /map endpoints under dataset URLs.";
  if (path.includes("/items") || path.includes("/features")) {
    detail +=
      " Use GET /api/collections/{collectionSlug}/datasets with query params search, tag_filters, limit, offset, omit (not q= on invented paths). Then GET /api/collections/{collectionSlug}/datasets/{datasetSlug}, then .../files/{fileSlug} for selected-version metadata and assets.";
